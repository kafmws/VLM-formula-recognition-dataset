...
def load_func(dataset_syntax, dataset_meta: DatasetMeta, **load_kwargs):
    ...
    if dataset_meta.dataset_name in filepaths:
        data = []
        with open(file=filepaths[dataset_meta.dataset_name], mode="r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                item = json.loads(line)
                # item["images"] = [aug(Image.open(item["images"][0]))]
                data.append(item)
...
register_dataset(
    DatasetMeta(
        hf_dataset_id="datav0_imgaug",  # 只是标识
        dataset_name="datav0_imgaug",
...
register_dataset(
    DatasetMeta(
        hf_dataset_id="datav0_5_imgaug",  # 只是标识
        dataset_name="datav0_5_imgaug",
        load_function=load_func
    )
)
...
register_dataset(
    DatasetMeta(
        hf_dataset_id="datav0_5_norm_imgaug",  # 只是标识
        dataset_name="datav0_5_norm_imgaug",
        load_function=load_func
    )
)
...
if __name__ == '__main__':
    ...
    args = TrainArguments(
        ...
        logging_steps=1,
        max_length=8192,
        output_dir="./swift_output/datav0_5_norm_aug",
        dataset_num_proc=2,
        ...
        swanlab_mode="cloud",
        swanlab_exp_name="datav0_5_norm_aug",
        metric="acc",
    )
