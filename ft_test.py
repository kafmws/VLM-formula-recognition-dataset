...
def load_func(dataset_syntax, dataset_meta: DatasetMeta, **load_kwargs):
    if dataset_meta.dataset_name == "datav0_imgaug":
        data = []
        with open(file="/root/dev/data/datav0/train.jsonl", mode="r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
